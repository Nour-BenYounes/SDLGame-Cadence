...
//left
	...
	p->bar_score=NULL;
	p->pos_vie.x=1600;p->pos_vie.y=55;
	p->pos_score.x=1550;p->pos_score.y=110;
	p->score=0;
	p->health=100;
	p->death=0;
	p->direction=0;
...
/*
SDL_Rect draw={p.pos_hero.x-back.x,p.pos_hero.y-back.y,p.pos_hero.w-back.w,p.pos_hero.h-back.h};





//conditions d'affichage: btiz on background
	//idle
	if(p.direction==0 && p.state==idle) SDL_BlitSurface(p.hero_right,&p.animheroRight[idle][p.frame_i],screen_p,&draw);
	else if(p.direction==1 && p.state==idle) SDL_BlitSurface(p.hero_left,&p.animheroLeft[idle][p.frame_i],screen_p,&draw);
	//idle1
	if(p.direction==0 && p.state==idle1) SDL_BlitSurface(p.hero_right,&p.animheroRight[idle1][p.frame_i1],screen_p,&draw);
	else if(p.direction==1 && p.state==idle1) SDL_BlitSurface(p.hero_left,&p.animheroLeft[idle1][p.frame_i1],screen_p,&draw);
	//idle2
	if(p.direction==0 && p.state==idle2) SDL_BlitSurface(p.hero_right,&p.animheroRight[idle2][p.frame_i2],screen_p,&draw);
	else if(p.direction==1 && p.state==idle2) SDL_BlitSurface(p.hero_left,&p.animheroLeft[idle2][p.frame_i2],screen_p,&draw);
	//walking
	if(p.direction==0 && p.state==walking) SDL_BlitSurface(p.hero_right,&p.animheroRight[walking][p.frame_wl],screen_p,&draw);
	else if(p.direction==1 && p.state==walking) SDL_BlitSurface(p.hero_left,&p.animheroLeft[walking][p.frame_wl],screen_p,&draw);
	//jumping
	if(p.direction==0 && p.state==jumping) SDL_BlitSurface(p.hero_right,&p.animheroRight[jumping][p.frame_j],screen_p,&draw);
	else if(p.direction==1 && p.state==jumping) SDL_BlitSurface(p.hero_left,&p.animheroLeft[jumping][p.frame_j],screen_p,&draw);
	//doublejumping
	if(p.direction==0 && p.state==doublejumping) SDL_BlitSurface(p.hero_right,&p.animheroRight[doublejumping][p.frame_dj],screen_p,&draw);
	else if(p.direction==1 && p.state==doublejumping) SDL_BlitSurface(p.hero_left,&p.animheroLeft[doublejumping][p.frame_dj],screen_p,&draw);
	//attacking
	if(p.direction==0 && p.state==attacking) SDL_BlitSurface(p.hero_right,&p.animheroRight[attacking][p.frame_a],screen_p,&draw);
	else if(p.direction==1 && p.state==attacking) SDL_BlitSurface(p.hero_left,&p.animheroLeft[attacking][p.frame_a],screen_p,&draw);
	//death
	if(p.direction==0 && p.state==die) SDL_BlitSurface(p.hero_right,&p.animheroRight[die][p.frame_d],screen_p,&p.pos_hero);
	else if(p.direction==1 && p.state==die) SDL_BlitSurface(p.hero_left,&p.animheroLeft[die][p.frame_d],screen_p,&p.pos_hero);
	//hit
	if(p.direction==0 && p.state==hit) SDL_BlitSurface(p.hero_right,&p.animheroRight[hit][p.frame_h],screen_p,&draw);
	else if(p.direction==1 && p.state==hit) SDL_BlitSurface(p.hero_left,&p.animheroLeft[hit][p.frame_h],screen_p,&draw);
	//crouching
	if(p.direction==0 && p.state==crouch) SDL_BlitSurface(p.hero_right,&p.animheroRight[9][p.frame_c],screen_p,&draw);
	else if(p.direction==1 && p.state==crouch) SDL_BlitSurface(p.hero_left,&p.animheroLeft[9][p.frame_c],screen_p,&draw);*/
}
...
void deplacerPerso (personne *p)
{
	...
	{
		if (p->pos_hero.x < 500) p->pos_hero.x+=(p->vitesse+p->acceleration);
	}
	else if(p->direction==1)//left
	{
		if (p->pos_hero.x > 50) p->pos_hero.x-=(p->vitesse+p->acceleration);
	}
...
void deplacerPerso2 (personne *p)
{
	...
	if(p->direction==0) //right
	{
		if (p->pos_hero.x < 1600) p->pos_hero.x+=(p->vitesse+p->acceleration);
	}
	else if(p->direction==1)//left
	{
		if (p->pos_hero.x > 900) p->pos_hero.x-=(p->vitesse+p->acceleration);
	}
...
{
	if(p->pos_hero.y <p->ground)//triggered by "fonction saut"
	{
		p->pos_hero.y+=10;	
	}
	//reached the usual y
	if(p->pos_hero.y>=p->ground && (p->state==jumping || p->state==doublejumping || p->state==die))
	{
		p->pos_hero.y=p->ground;
		if(p->state!=die)
			p->state=idle;
	}
}
